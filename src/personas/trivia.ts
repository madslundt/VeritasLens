...
2. Include a short verbatim quote (≤140 chars) from the audio that captures how the question was asked.
3. Provide the correct, definitive answer in one short phrase (max 60 characters).
4. Provide one brief explanatory sentence (max 180 characters) with an interesting supporting fact.
...
export function buildTriviaPrompt(lang: LanguageCode): string {
  ...
  return `${TRIVIA_BASE_PROMPT}\n\nLANGUAGE: Write \`question\`, \`answer\`, and \`description\` in ${langName}. \`quote\` stays in the original spoken language.`;
}
