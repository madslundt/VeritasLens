...
**Goal:** Replace README.md with an accurate, fully-rewritten version that reflects the current codebase — 9 built-in lenses, no custom lenses, session history, configurable buffer, auto-summary, and dual-model config.

**Architecture:** Single file change (`README.md`). No source code, tests, or config touched. All section headings preserved from the original.
...
Confirm the 10 existing sections are present: intro paragraph, Features, Gestures, Architecture, Quick Start, Tests + Build, Release, Privacy, Tech Notes, License footer.
...
```bash
git add README.md
git commit -m "docs: rewrite README to match current codebase

- Replace single fact-checker + custom lenses with 9 built-in lenses
- Update gestures table: double-tap is universal, history pages added
- Update file tree with all persona files
- Add: configurable buffer, session history, auto-summary, dual-model config
- Update Tech Notes: remove custom-lens note, add Auto lens two-call note"
```
