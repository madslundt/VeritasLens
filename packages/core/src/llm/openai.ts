...
export interface CallOpenAiLensOptions {
   ...
   * an `image_url` content part (data URI) so vision-capable models can use
   * both the audio transcript and the camera frame. The Even G2 app never
...
export async function callOpenAiLens(opts: CallOpenAiLensOptions): Promise<string> {
  ...
  if (OPENAI_INLINE_AUDIO_HOSTS.has(opts.baseUrl)) {
    userContent = [
      ...
      ...(opts.imageData ? [{ type: 'image_url', image_url: { url: `data:image/jpeg;base64,${opts.imageData}` } }] : []),
    ...
    if (opts.imageData) {
      userContent = [
        { type: 'image_url', image_url: { url: `data:image/jpeg;base64,${opts.imageData}` } },
        { type: 'text', text: transcriptUserMessage(transcript) },
      ];
    } else {
