...
export interface CallClaudeLensOptions {
  ...
  tools?: unknown[];
  /**
   * Optional base64-encoded JPEG image frame. When provided it is forwarded as
   * an image content block alongside the transcript so vision-capable Claude
   * models can use both modalities. The Even G2 app never passes this field.
   */
  imageData?: string;
}
...
export async function callLens(opts: CallClaudeLensOptions): Promise<string> {
  ...
  const body = {
    ...
    messages: [{
      role: 'user',
      content: opts.imageData
        ? [
            ...(opts.imageData ? [{ type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: opts.imageData } }] : []),
            { type: 'text', text: transcriptUserMessage(opts.transcript) },
          ]
        : transcriptUserMessage(opts.transcript),
    }],
  };
...
export async function callLensStream(opts: CallClaudeLensStreamOptions): Promise<string> {
  ...
  const body = {
    ...
    messages: [{
      ...
      content: opts.imageData
        ? [
            { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: opts.imageData } },
            { type: 'text', text: transcriptUserMessage(opts.transcript) },
          ]
        : transcriptUserMessage(opts.transcript),
    }],
