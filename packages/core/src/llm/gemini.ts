...
export interface CallLensOptions {
  ...
  tools?: unknown[];
  /**
   * Optional base64-encoded JPEG image frame (e.g. from a Ray-Ban Meta camera).
   * When provided it is forwarded as an `inlineData` part alongside the audio so
   * vision-capable models can use both modalities. The Even G2 app never passes
   * this field — no breaking change.
   */
  imageData?: string;
}
      ...
      contents: [
        {
          ...
          parts: [
            ...
            ...(opts.imageData ? [{ inlineData: { mimeType: 'image/jpeg', data: opts.imageData } }] : []),
          ],
        },
      ],
      generationConfig: {
