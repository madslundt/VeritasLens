...
import { callLens, configureSettings as coreConfigureSettings } from '@veritaslens/core';
import { chooseClassifierModel } from '@/runtime/lifecycle';
import { loadSettings, settings } from '@/state/store';
import {
...
// Wire the even-g2 Solid settings signal into core's settings accessor so that
// `callLens` (which reads `settings()` from core's store) picks up the values
// set by `loadSettings()` in this test file.
coreConfigureSettings(() => settings());
