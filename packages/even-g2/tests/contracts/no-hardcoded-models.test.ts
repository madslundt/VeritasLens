...
//
// `types.ts` is the single source of truth for which models the app
// knows about. Any module under `llm/`, `runtime/`, `personas/`,
// `state/`, or the entrypoint `src/main.tsx` that hardcodes a
...
//
// `views/` is explicitly out of scope: settings UI legitimately
...
const MONO_ROOT = join(PKG_ROOT, '..', '..');

// In the monorepo the source of truth for model names lives in packages/core/src/types.ts.
// LLM call-site code lives across both packages.
const CORE_SRC = join(MONO_ROOT, 'packages', 'core', 'src');
const EVEN_G2_SRC = join(PKG_ROOT, 'src');

const ALLOWED_FILES = new Set<string>([
  // The single source of truth for known model names + their patterns.
  join(CORE_SRC, 'types.ts'),
]);

// Subtrees we scan. `views/` is out of scope (renders UI placeholders
// and examples to the user). Everything that *calls* the LLM lives here.
const SCANNED_SUBTREES = [
  join(CORE_SRC, 'llm'),
  join(CORE_SRC, 'runtime'),
  join(CORE_SRC, 'personas'),
  join(CORE_SRC, 'state'),
  join(EVEN_G2_SRC, 'runtime'),
  join(EVEN_G2_SRC, 'state'),
];
const SCANNED_FILES = [join(EVEN_G2_SRC, 'main.tsx')];
...
describe('no hardcoded model names in LLM call-site code', () => {
  for (const pattern of MODEL_PATTERNS) {
    it(`no LLM call-site file contains a literal ${pattern.name}`, () => {
      ...
      expect(
        offenders,
        `found ${pattern.name} literal outside types.ts:\n${offenders
